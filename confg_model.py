...
class CodeBERT4JIT(nn.Module):
    def __init__(self, args):
        ...
        if self.freeze_n_layers>0:
            print("Freeze the first {} layers of RoBERTa".format(self.freeze_n_layers))
        ...
        if self.reinit_pooler:
            print("Reinit the pooler layer of RoBERTa")
            self.sentence_encoder.pooler.apply(self.init_weights)

            # 初始化前n层模型参数为正态分布
        if self.reinit_n_layers > 0:
            print("Reinit the first {} layers of RoBERTa".format(self.reinit_n_layers))

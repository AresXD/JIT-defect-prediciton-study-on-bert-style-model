...
import numpy as np
import torch
from transformers import BertTokenizer, BertModel,  RobertaModel, RobertaTokenizer,AutoTokenizer,AutoModelForTokenClassification
...
def tokenization_for_codebert(data, max_length, flag,params):
    ...
    # vul_tokenizer=AutoTokenizer.from_pretrained("mrm8488/codebert-base-finetuned-detect-insecure-code")
    # vulcode_toknizer=RobertaTokenizer.from_pretrained("")
    if flag == 'msg':
    	features = convert_examples_to_features(examples = data, max_seq_length=max_length, tokenizer=tokenizer)
    	return features
    elif flag == 'code':
        features =  convert_examples_to_hierarchical_features(examples = data, max_seq_length=max_length, tokenizer=tokenizer,params=params)
        return features
    else:
